...
func (f FriendshipFunc) Mutate(ctx context.Context, m codegen.Mutation) (codegen.Value, error) {
	if mv, ok := m.(*codegen.FriendshipMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *codegen.FriendshipMutation", m)
}
