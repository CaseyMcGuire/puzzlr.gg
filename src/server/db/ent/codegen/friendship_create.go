...
func (_c *FriendshipCreate) check() error {
	if _, ok := _c.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`codegen: missing required field "Friendship.user_id"`)}
	}
	if _, ok := _c.mutation.FriendID(); !ok {
		return &ValidationError{Name: "friend_id", err: errors.New(`codegen: missing required field "Friendship.friend_id"`)}
	}
	if len(_c.mutation.UserIDs()) == 0 {
		return &ValidationError{Name: "user", err: errors.New(`codegen: missing required edge "Friendship.user"`)}
	}
	if len(_c.mutation.FriendIDs()) == 0 {
		return &ValidationError{Name: "friend", err: errors.New(`codegen: missing required edge "Friendship.friend"`)}
	}
	return nil
}
